...
; PUSHADした値をシステムのスタックにコピーする
	MOV	EDX,[ESP   ]
	MOV	EBX,[ESP+ 4]
	MOV	[ECX   ],EDX	; hrb_apiに渡すためコピーする
	MOV	[ECX+ 4],EBX	; hrb_apiに渡すためコピーする
	MOV	EDX,[ESP+ 8]
	MOV	EBX,[ESP+12]
	MOV	[ECX+ 8],EDX	; hrb_apiに渡すためコピーする
	MOV	[ECX+12],EBX	; hrb_apiに渡すためコピーする
	MOV	EDX,[ESP+16]
	MOV	EBX,[ESP+20]
	MOV	[ECX+16],EDX	; hrb_apiに渡すためコピーする
	MOV	[ECX+20],EBX    ; hrb_apiに渡すためコピーする
	MOV	EDX,[ESP+24]
	MOV	EBX,[ESP+28]
	MOV	[ECX+24],EDX	; hrb_apiに渡すためコピーする
	MOV	[ECX+28],EBX    ; hrb_apiに渡すためコピーする
