		...
		if (fifo32_status(&task->fifo) == 0) {
			...
			{	/* カーソルOFF */
				boxfill8( sheet->buf, sheet->bxsize, COL8_000000, 
					cons.cur_x, cons.cur_y, cons.cur_x + 7, cons.cur_y + 43 );
				cons.cur_c = -1;
			}
			if (256 <= i && i <= 511) {
				if (i == 8 + 256) {
...
void cons_runcmd( char * cmdline, struct CONSOLE * cons, int * fat, unsigned int memtotal )
{
	...
	{	/* コマンドでなく, 空白行でもない */
		putfonts8_asc_sht( cons->sht, 8, cons->cur_y, 
				COL8_FFFFFF, COL8_000000, "Bad comment.", 12 ); 
		cons_newline( cons );
		cons_newline( cons );
	}
	return;
...
void cmd_hlt( struct CONSOLE * cons, int * fat )
{
	...
	{	/* ファイルが見つからなかったばあい */
		putfonts8_asc_sht( cons->sht, 8, cons->cur_y, COL8_FFFFFF, COL8_000000, "File now found.", 15 );
		cons_newline( cons );
	}
	cons_newline( cons );
	return;
